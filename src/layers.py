...
class AttentionModule(torch.nn.Module):
    ...
    def setup_weights(self):
        ...
        self.weight_matrix = torch.nn.Parameter(
            torch.Tensor(self.args['Parameters']['filters_3'], self.args['Parameters']['filters_3'])
        )
...
class DenseAttentionModule(torch.nn.Module):
    """
    GNN Dense Attention Module to make a pass on graph.
    """
...
class TensorNetworkModule(torch.nn.Module):
    """
    GNN Tensor Network module to calculate similarity vector.
    """
    ...
    def setup_weights(self):
        ...
        self.weight_matrix = torch.nn.Parameter(
            torch.Tensor(
                self.args['Parameters']['filters_3'], self.args['Parameters']['filters_3'], self.args['Parameters']['tensor_neurons']
            )
        )
        self.weight_matrix_block = torch.nn.Parameter(
            torch.Tensor(self.args['Parameters']['tensor_neurons'], 2 * self.args['Parameters']['filters_3'])
        )
        self.bias = torch.nn.Parameter(torch.Tensor(self.args['Parameters']['tensor_neurons'], 1))
    ...
    def forward(self, embedding_1, embedding_2):
        ...
        #(self.weight_matrix.view(self.args['Parameters']['filters_3'], -1).shape) # 原始输入的两个实体都是16维向量，现在用256维表示他们的某种关系
        scoring = torch.matmul(
            embedding_1, self.weight_matrix.view(self.args['Parameters']['filters_3'], -1)
        )
        #print(scoring.shape)
        scoring = scoring.view(batch_size, self.args['Parameters']['filters_3'], -1).permute([0, 2, 1]) #filters_3可以理解成找多少种关系
        #print(scoring.shape)
        scoring = torch.matmul(
            scoring, embedding_2.view(batch_size, self.args['Parameters']['filters_3'], 1)
        ).view(batch_size, -1)
        #print(scoring.shape)
...
class DiffPool(torch.nn.Module):
    def __init__(self, args, num_nodes=10, num_layers=4, hidden=16, ratio=0.25):
        ...
        self.args = args
        num_features = self.args['Parameters']['filters_3']
